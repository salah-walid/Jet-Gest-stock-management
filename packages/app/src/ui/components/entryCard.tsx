...
interface Props{
    entry: Entries;
    index: number;
    editEntry?: (id: number, entry: Entries) => void;
    allowModification: boolean;
    changePosition?: (from: number, to: number) => void;
    onDelete?: (index: number) => void;
}

const EntryCard : FC<Props> = (props) => {
    let entry = Object.assign({}, props.entry);
    ...
    return (
        <tr onClick={() => Routes.navigator.push(`${Routes.productDetail}/${props.entry.product.id}`)}>
            {
                ...
                : <>
                    <th scope="row">{props.index}</th>
                    <td>{props.entry.product.title}</td>
                    <td>{props.entry.unitPrice.toLocaleString('fr-FR', {minimumIntegerDigits: 2, useGrouping:false})}</td>
                    <td>{props.entry.quantity}</td>
                    <td>{props.entry.price.toLocaleString('fr-FR', {minimumIntegerDigits: 2, useGrouping:false})}</td>
                </>
            }
        </tr>
    )
