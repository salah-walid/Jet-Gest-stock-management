...
import { OrderRandoPageCombo } from '../../../core/models/orderRandoData';

class OrderRandoViewModel{
    ...
    public orders : OrderRandoPageCombo = {
        pageCount : 1,
        order : [],
    }
    ...
    public async getPossibleSearchKeys() {
        this.possibleSearchKeys = await this._apiService?.getData<any>("gestion/getOrderRandoPossibleSearchKeys") ?? this.possibleSearchKeys;
    ...
    public async getOrders() {
        let orders: OrderRandoPageCombo | null = null;
        if(!this.isSearching)
            orders = (await this._apiService
                    ?.postData<OrderRandoPageCombo>(`gestion/getRandoOrders/${this.currentPage}`, {"sellerId": this.chosenSeller}))?.data
                    ?? this.orders;
        else{
            let search = Array.from<SearchParams, string>(this.search, (item) => {return item.key + "=" + item.value});
            orders = (await this._apiService
                    ?.postData<OrderRandoPageCombo>(`gestion/searchRandoOrders/${this.currentPage}`, {"search": search, "sellerId": this.chosenSeller}))?.data
                    ?? this.orders;
        }
    ...
    public async deleteOrder(id: number | null | undefined) {
        ...
        this._apiService.deleteData(`gestion/deleteRandoOrder/${id}`, {"Authorization" : `TOKEN ${this._authService.token}`});
    }
}

export default new OrderRandoViewModel;
