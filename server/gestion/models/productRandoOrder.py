...
class ProductRandoOrder(models.Model):

    quantity = models.IntegerField(default=0)
    unitPrice = models.FloatField(default=0)

    product = models.ForeignKey(Product,null=False, on_delete=models.PROTECT)
    order = models.ForeignKey(OrderRando, null=False, on_delete=models.CASCADE)
...
@receiver(pre_save, sender=ProductRandoOrder)
def _post_save_receiver(sender, instance: ProductRandoOrder, using, **kwargs):
    if using == "remote":
        return
    from .SalesAndBalance import SalesAndBalance
    sab, c = SalesAndBalance.objects.get_or_create(chooseProduct=instance.product)
    try:
        prev = ProductRandoOrder.objects.get(id=instance.id)
...
@receiver(pre_delete, sender=ProductRandoOrder)
def _pre_delete_receiver(sender, instance: ProductRandoOrder, using, **kwargs):
    if using == "remote":
        return
    from .SalesAndBalance import SalesAndBalance
    sab, c = SalesAndBalance.objects.get_or_create(chooseProduct=instance.product)
